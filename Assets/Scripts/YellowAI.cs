...
public class YellowAI : MonoBehaviour
{
    ...
    private float projectilesShot = 0;
    ...
    void Patrolling()
    {
        ...
        if (Vector3.Distance(target.position, this.transform.position)
            <= minCombatDistance && attackDelay >= 3)
        {
            attackDelay = 0;
            currentState = State.Attacking;
        }
    ...
    void SpinAndShoot()
    {
        rb.rotation += 75f * Time.deltaTime;
        Shoot();
    }
    ...
    void Teleport()
    {
        teleport.x = Random.Range(-3.5f, 3.5f);
        teleport.y = Random.Range(-3.5f, 3.5f);
